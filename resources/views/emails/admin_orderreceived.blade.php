<table>
    ...
    <tr>
        <td colspan='2'>
            {{ __('emails.order_admin.line_hello') }},<br/><br/>
            {{ __('emails.order_admin.line_1',['date'=>date('m-d-Y', strtotime($order_detail["order"]["purchase_date"])),'name'=>$order_detail["user"]["first_name"]]) }} 
        </td>
    </tr>
    <tr>
        <td colspan="2">&nbsp;</td>
            ...
            <table width="100%">
                <tr>
                    <td width="30%" align="left" valign="top">
                        <strong>{{ __('emails.order_admin.line_2') }}</strong>
                    </td>
            ...
            <table width="100%">
                <tr>
                    <td width="{{ $order_detail['order']['allow_shipping']=='true'? '50%' : '100%'}}" align="left" valign="top">
                        <strong>{{ __('emails.order_admin.line_3') }}</strong>
                    </td>
                    @if($order_detail['order']['allow_shipping']=="true")
                    <td width="50%" align="left" valign="top">
                        <strong>{{ __('emails.order_admin.line_4') }}</strong>
                    </td>
                    @endif
                </tr>
            </table>
    ...
    <tr>
        <td colspan='2'><strong>{{ __('emails.order_admin.line_5') }}</strong></td>
    </tr>
        ...
        <td colspan='2' width="100%">
            <table width="100%">
                <tr>
                    ...
                    {{ __('emails.order_admin.line_6') }}
                    </th>
                    <th width="10%" align="center">
                    {{ __('emails.order_admin.line_7') }}
                    </th>
                    <th width="20%" align="right">
                    {{ __('emails.order_admin.line_8') }}
                    </th>
                </tr>
                ...
                <tr>
                    ...
                    <td valign="top" align="center">
                        {{ $product["name"] }}<br/>
                        {{ __('emails.order_admin.line_9') }}: {{ $product["size"] }}<br/>
                        {{ __('emails.order_admin.line_10') }}: {{ $product["color"] }}
                    </td>
                    <td valign="top" align="center">
                        {{ $product["quantity"] }}
                    </td>
                ...
                <tr>
                    <td colspan="3" align="right">
                        <strong>{{ __('emails.order_admin.line_11') }}</strong>
                    </td>
                    <td align="right">
                ...
                <tr>
                    ...
                    <td align="right">
                        @if($order_detail['order']['allow_shipping']=="true")
                        ${{ number_format(($order_detail["order"]["order_total"] + ($order_detail["order"]["order_total"] * $order_detail["order"]["order_tax"]) + $order_detail["order"]["shipping_total"]) - $order_detail["order"]["discount_app"],'2','.',',') }}
                        @else
                        ${{ number_format(($order_detail["order"]["order_total"] + ($order_detail["order"]["order_total"] * $order_detail["order"]["order_tax"]))-$order_detail["order"]["discount_app"],'2','.',',') }}
                        @endif
                    </td>
                </tr>
            </table>
